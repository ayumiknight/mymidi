...
4. config object ( optional )
	{
		...
		anchorLeft: the left anchor of x scroll
		anchorRight: the right anchor of x scroll,
		isPagedX: if this is a horizontal paged scroll,
		isPagedY: if this is a vertical paged scroll,
		onPageChanged: the callback function when page changed ( need isPagedX or isPagedY ),
		onFirstMove: the callback function which will be triggered when the move start at the frist time ( only trigger one time ),
		moveThresh: the threshhold value needed when detecting the swipe direction
	}
...
function ScrollPane(el, scrollHorizontal, scrollVertical, options)
{
	...
	var Sys = function()
	{
		var a = navigator.userAgent,
			b = a.match(/iPhone/i),
			c = a.match(/iPad/i),
			isWechat = a.match(/MicroMessenger/i),
			h = a.match(/iPod/i),
			h = b || c || h,
			e = /Android/.test(a),
			d = /WebKit/.test(a),
	...
	SPZ.RBand.prototype.update = function(isDraggingPos, dragX, targetX)
	{
		if (isDraggingPos)
		{
			this.v = dragX - this.p;
	...
	SPZ.ScrollPane.prototype.dragChange = function(a, b, evt) 
	{
		this.dragX = a;
		this.dragY = b;
	...
	SPZ.ScrollPane.prototype.dragEnd = function(a, b, c)
	{
		...
		if (this.anchorLeft)
		{
			var _x = Math.abs(this.contentX);
			var _a = Math.abs(this.anchorLeft);
			this.targetX = this.contentX*this.anchorLeft>0 && _x > _a ? this.anchorLeft : 0;
			anchorOn = this.targetX == this.anchorLeft;
		}
		if (this.anchorRight && !anchorOn)
		{
			var _x = Math.abs(this.contentX);
			var _a = Math.abs(this.anchorRight);
			this.targetX = this.contentX*this.anchorRight<0 && _x > _a ? -1*this.anchorRight : 0;
		}
		this.play();
	};
