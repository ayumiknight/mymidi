...
const HotelDatePicker = React.createClass({
				...
				<a href="javascript:;" className="close" {...z.tap(this.props.onClose)}>
					<svg className="icon icon-21" viewBox="0 0 1050 1050">
						<path className="path1 fill-color3"
						      d="M1050.041 43.017l-43.037-43.017-469.004 468.983-468.983-468.983-43.017 43.017 468.983 468.983-468.983 468.983 43.017 43.017 468.983-468.983 469.004 468.983 43.037-43.017-468.983-468.983z"></path>
					</svg>
				</a>
				<div className="inner">
					<h2>{this.props.title||"选择日期"}</h2>
				</div>
	...
	getDayDom(today, idx) {
		...
		if (Helper.compare_date(this.state.dateTo, today) < 0) valid = false;
		...
		return (<td className="day-cell clickable" key={today.toString()} {...z.tap(() => {
			if (valid) this.props.onChoose(Helper.date2str(today), today);
		})}>
			<div className={classes.join(' ')}>
				<span className="date">{today.getDate().toString()}</span>
				<span className="extra">{info.text}</span>
			</div>
		</td>);
	}
